...
import { FiFileText, FiClipboard, FiHelpCircle, FiEdit, FiArrowLeft, FiTrash2 } from 'react-icons/fi';
...
type ActiveTab = 'summary' | 'flashcards' | 'practice' | 'mcq';
...
export default function NoteDetailPage() {
  ...
  const handleGenerate = async (type: 'summary' | 'flashcards' | 'categorized', options?: any) => {
                ...
                <div className="mt-4">
                  {note.categorizedQuestions[activePracticeTab as keyof typeof note.categorizedQuestions]?.map((q, index) => (
                    <Accordion key={index} title={q.question}>
                      <div className="prose dark:prose-invert max-w-none" dangerouslySetInnerHTML={{ __html: q.answer }} />
                    </Accordion>
                  ))}
                  <div className="text-center mt-6">
                    <Button 
